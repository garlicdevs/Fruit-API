...
class OpenSim(Environment):
    ...
    def __init__(self, level, visualize=False, integrator_accuracy=5e-5):
        from osim.env import L2M2019Env, Arm2DEnv, OsimEnv

        environments = dict(Arm2D=Arm2DEnv, L2Run=L2M2019Env, Prosthetics=OsimEnv)
    ...
    def execute(self, actions):
        states, reward, terminal, _ = self.environment.step(action=actions)
