...
class TensorforceModel(Model):
    ...
    def __init__(
        ...
        #print('\tpolicy_cls, first_arg, kwargs', policy_cls, first_arg, kwargs)
        ...
        if baseline_policy is None:
            ...
            baseline_cls, first_arvalues_specg, kwargs = Module.get_module_class_and_kwargs(
            ...
            #print('\tbaseline_policy', baseline_policy, 'cls', baseline_cls, first_arg, kwargs)
        ...
        #print('\tupdate_frequency', self.update_frequency, self.update_start)
        ...
        #print('\tbaseline_optimizer', baseline_optimizer)
        ...
        #print('\tbaseline objective', baseline_objective)
    ...
    def tf_core_act(self, states, internals, auxiliaries):
        ...
        #print('TensorForceModel <-- tf_core_act()\n')
        ...
        with tf.control_dependencies(control_inputs=(optimized,)):
            ...
            if self.is_summary_logged(
                ...
                kl_divergences = self.policy.kl_divergence(
                    states=states, internals=internals, auxiliaries=auxiliaries,
                    other=kldiv_reference, include_per_action=True
                )
            ...
            if self.is_summary_logged(label=('action-kl-divergences', 'kl-divergences')):
                for name in self.actions_spec:
                    optimized = self.add_summary(
    ...
    def tf_optimize_baseline(self, indices):
        ...
        with tf.control_dependencies(control_inputs=(optimized,)):
            ...
            if self.is_summary_logged(label=('baseline-loss', 'losses')):
                loss = objective_loss + regularization_loss
                optimized = self.add_summary(
                    label=('baseline-loss', 'losses'), name='loss', tensor=loss,
                    pass_tensors=optimized
                )
