...
class Agent(object):
    ...
    def __init__(
        ...
        # print('\nAgent --> init()')
        # print('\tstate', states, 'actions', actions, 'max_episode_timesteps', max_episode_timesteps,
        #       'parallel interactions', parallel_interactions, 'buffer observe', buffer_observe,
        ...
        # print('\tstates spec', self.states_spec, 'action spec', self.actions_spec, 'max episode timesteps',
        #       self.max_episode_timesteps, 'parallel interactions', self.parallel_interactions,
        #       'buffer observe', self.buffer_observe, 'seed', seed, 'recorder', self.recorder_spec)
        # print('Agent <-- init()\n')
    ...
    def reset(self):
        ...
        #print('\ttime steps', self.timesteps, 'episodes', self.episodes, 'updates', self.updates)
        #print('Agent <-- reset()\n')
    ...
    def act(
        ...
        #print('\tBatch states', states, auxiliaries)

        # Model.act()
        #print('\tmodel type', type(self.model), self.model.act)
        if query is None:
        ...
        #print('\tReverse normalized actions', actions)
    ...
    def observe(self, reward, terminal=False, parallel=0, query=None, **kwargs):
        ...
        # print('\tBuffer indices', parallel, index)
        ...
        if terminal > 0 or index == self.buffer_observe or query is not None:
            ...
            # print('\tself.updated', self.updates)
            # print('\tquery')
